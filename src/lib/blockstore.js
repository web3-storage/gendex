 ...
 * @typedef {import('cardex/multihash-index-sorted/api').MultihashIndexItem} MultihashIndexItem
 * @typedef {string} MultihashString
 * @typedef {{ cid: UnknownLink, bytes: Uint8Array }} Block
...
export class R2Blockstore {
   ...
   * @param {import('./car-index').CarIndexer} index
   */
...
export class BatchingR2Blockstore extends R2Blockstore {
  ...
  async #processBatch () {
    ...
    while (true) {
      ...
      const carPath = `${origin}/${origin}.car`
      const range = {
        offset: batch[0].offset,
        length: batch[batch.length - 1].offset - batch[0].offset + MAX_ENCODED_BLOCK_LENGTH
      }

      console.log(`fetching ${batch.length} blocks from ${origin} (${range.length} bytes @ ${range.offset})`)
      const res = await this._dataBucket.get(carPath, { range })
      if (!res) {
      ...
      while (true) {
        try {
          ...
          if (blocks) {
            ...
            const block = { cid: blockHeader.cid, bytes: bytes.slice() }
